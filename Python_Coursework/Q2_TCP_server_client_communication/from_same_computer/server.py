...
if a new client connects to the server, the state will turn to 'off' to close this 
unique thread.
'''
...
def from_client(socket_tcp_server):
    ...
    print(f"Ji Chonggui is connected to Server, its address and port are {client_addr}"
    ...
    while True:           # using while loop to make server can always receive and send messages until the client close the connection
        recv_data = new_client_socket.recv(1024)  # receive the data from the client, maximum 1024 bytes
        if recv_data:
        ...
        new_client_socket.send(send_data.encode("gbk"))    # send the input message to the client
    new_client_socket.close()      # close this unique socket which is connecting
    print("Client closed connection")    # show something to remind the server
...
def main():
    ...
    while True:
        if client_flag == 'on':  # 当client_flag为1时，即服务器得到客户端请求连接时，开始一个新的线程
            ...
            client_flag = 'off'  # 标志为，目的使线程不会一直增多，只有当服务器得到客户端请求连接时，才开始一个新的线程
            # the "off" means that the number of thread will not increase all the time
